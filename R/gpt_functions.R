...
get_system_prompt <- function(seu_obj) {
  ...
  unified_prompt <- paste(
    ...
    "  - For CoveragePlot when the user asks to show a specific cell type: NEVER subset() the Seurat object.",
    "    Instead, build a cells vector and pass it via the `cells` parameter:",
    "    BAD:  seu_sub <- subset(seu_obj, subset = Annotation_Broad == 'TRPV1_Peptidergic'); CoveragePlot(object = seu_sub, ...)",
    "    GOOD: cells_use <- colnames(seu_obj)[seu_obj$Annotation_Broad == 'TRPV1_Peptidergic']; CoveragePlot(object = seu_obj, cells = cells_use, group.by = ..., ...)",
    "    The full seu_obj must be passed as `object`; group.by still works normally on top of cells.",
    "",
    "Rules for message:",
